...
namespace Nailpod.ViewModels
{
    public class ValidateConnectionViewModel : ViewModelBase
    {
        ...
        public async Task ExecuteAsync(string connectionString, DataProviderType dataProvider)
        {
            try
            {
                if (dataProvider == DataProviderType.SQLServer)
                {
                    using (var db = new SQLServerDb(connectionString))
                    {
                        var dbCreator = db.GetService<IDatabaseCreator>() as RelationalDatabaseCreator;
                ...
                else if (dataProvider == DataProviderType.Maria)
                {
                    using (var db = new MariaDb(connectionString))
                    {
                        var dbCreator = db.GetService<IDatabaseCreator>() as RelationalDatabaseCreator;
                        if (await dbCreator.ExistsAsync())
                        {
