...
namespace Nailpod.ViewModels
{
    ...
    public class OrderListViewModel : GenericListViewModel<OrderModel>
    {
        ...
        protected override async void OnNew()
        {
            if (IsMainView)
            {
                await NavigationService.CreateNewViewAsync<OrderDetailsViewModel>(new OrderDetailsArgs { PlaceID = ViewModelArgs.PlaceID });
            }
            else
            {
                NavigationService.Navigate<OrderDetailsViewModel>(new OrderDetailsArgs { PlaceID = ViewModelArgs.PlaceID });
            }
        ...
        private DataRequest<Order> BuildDataRequest()
        {
            ...
            if (ViewModelArgs.PlaceID > 0)
            {
                request.Where = (r) => r.place_id == ViewModelArgs.PlaceID;
            }
            return request;
        }
