            ...
            if (args != null)
            {
                switch (args.Uri.AbsolutePath.ToLowerInvariant())
                {
                    ...
                    case "customers":
                        ...
                        if (customerID > 0)
                        {
                            return ActivationInfo.Create<PlaceDetailsViewModel>(new CustomerDetailsArgs { CustomerID = customerID });
                        }
                        return ActivationInfo.Create<PlacesViewModel>(new CustomerListArgs());
                    case "order":
                    case "orders":
                        ...
                        return ActivationInfo.Create<PlacesViewModel>(new OrderListArgs());
                    case "product":
                    case "products":
